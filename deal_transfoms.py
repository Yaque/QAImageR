...
def test(path, file):
    ...
    model.load_state_dict(torch.load("m/qq7_95.83333333333333_qq.pkl"))
    imagexx = Image.open(path)
    ...
    image = transform(imagexx).unsqueeze(0)
    ...
    for i, c in enumerate(predicted_test[0]):
        end_data += characters[c]
    print("==============识别结果：", end_data)
    # print("==============标准结果：", label_text)
    imagexx.save("TestTT/" + end_data[0:3] + "_____" + file)
